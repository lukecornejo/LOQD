...
extern double **phi , **phi_x , **phi_y , **j_x , **j_y; // scalar flux and current solution
extern double **phiT, **phi_xT, **phi_yT, **j_xT, **j_yT; // scalar flux and current from transport
extern double **phiL, **phi_xL, **phi_yL; // scalar flux from last iteration
extern double **E_xx  , **E_yy  , **E_xy;
extern double **E_xx_x, **E_yy_x, **E_xy_x;
extern double **E_xx_y, **E_yy_y, **E_xy_y;
extern double ***psiL, ***psiR, ***psiB, ***psiT;
extern double *cL, *cR, *cB, *cT;
extern double *phiOutL, *phiOutR, *phiOutB, *phiOutT;
extern double *phiInL, *phiInR, *phiInB, *phiInT;
extern double *jInL, *jInR, *jInB, *jInT;
...
void initialize()
{
	...
	cL=new double[ny]; phiInL=new double[ny]; phiOutL=new double[ny]; jInL=new double[ny];
	cR=new double[ny]; phiInR=new double[ny]; phiOutR=new double[ny]; jInR=new double[ny];
	cB=new double[nx]; phiInB=new double[nx]; phiOutB=new double[nx]; jInB=new double[nx];
	cT=new double[nx]; phiInT=new double[nx]; phiOutT=new double[nx]; jInT=new double[nx];
	...
	for (i=0; i<nx; i++) {
		...
		phi[i] =new double[ny];
		E_xx[i]=new double[ny];
		E_yy[i]=new double[ny];
		E_xy[i]=new double[ny];
		psi[i] =new double[ny];
	}
	// cell edge values on x grid
	phi_xT=new double*[nx+1];
	phi_xL=new double*[nx+1];
	phi_x =new double*[nx+1];
	j_x   =new double*[nx+1];
	j_xT  =new double*[nx+1];
	E_xx_x=new double*[nx+1];
	E_yy_x=new double*[nx+1];
	E_xy_x=new double*[nx+1];
	psi_x =new double*[nx+1];
	for (i=0; i<nx+1; i++) {
		phi_xT[i]=new double[ny];
		phi_xL[i]=new double[ny];
		phi_x[i] =new double[ny];
		j_x[i]   =new double[ny];
		j_xT[i]  =new double[ny];
		E_xx_x[i]=new double[ny];
		E_yy_x[i]=new double[ny];
		E_xy_x[i]=new double[ny];
		psi_x[i] =new double[ny];
	}
	// cell edge flux on x grid
	phi_yT=new double*[nx];
	phi_yL=new double*[nx];
	phi_y =new double*[nx];
	j_y   =new double*[nx];
	j_yT  =new double*[nx];
	E_xx_y=new double*[nx];
	E_yy_y=new double*[nx];
	E_xy_y=new double*[nx];
	psi_y =new double*[nx];
	for (i=0; i<nx; i++) {
		phi_yT[i]=new double[ny+1];
		phi_yL[i]=new double[ny+1];
		phi_y[i] =new double[ny+1];
		j_y[i]   =new double[ny+1];
		j_yT[i]  =new double[ny+1];
		E_xx_y[i]=new double[ny+1];
		E_yy_y[i]=new double[ny+1];
		E_xy_y[i]=new double[ny+1];
