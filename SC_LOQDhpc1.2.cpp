...
/*
  ...
  ******************************************************
  ...
  To complile
  Windows
c++ -I .\eigen SC_LOQDhpc1.2.cpp iotrans.cpp def.cpp -o SC_LOQDhpc1.2.exe -std=c++0x
  Linux
  
  hpc
c++ -I .\eigen -O3 SC_LOQDhpc1.2.cpp iohpc.cpp def.cpp -o SC_LOQDhpc1.2b.exe
-O
-O2
-O3 better
-Os
...
icpc -I .\eigen -O3 SC_LOQDhpc1.2.cpp iohpc.cpp def.cpp -o SC_LOQDhpc1.2a.exe
-O1
-O2
-O3 best
-xO
...
void LOQDsolution()
{
	...
	for (j=0; j<ny; j++) {
		...
		p++; c++;
		for (i=1; i<nx; i++) {
			A.insert(p,c-1) = -2*hy[j]*E_xx[i-1][j]*sigmaT[i][j]/hx[i-1];  A.insert(p,c) = -2*hy[j]*E_xx[i][j]*sigmaT[i-1][j]/hx[i]; // centre flux
...
void output(char fn[])
{
	...
	outfile<<endl;
	for (i=0; i<nx; i++) outfile<<setw(6)<<i+1<<print_out(x[i])<<print_out((x[i]+x[i+1])/2)<<print_out(bcB[i])<<print_out(bcT[i])<<endl; // write x grid
	outfile<<setw(6)<<nx+1<<print_out(x[nx])<<endl;
	outfile<<" Y grid \n"<<" Index     Cell Edge      Cell Centre  ";
	switch ( kbc ) {
	case 1:
	...
	outfile<<endl;
	for (j=0; j<ny; j++) outfile<<setw(6)<<j+1<<print_out(y[j])<<print_out((y[j]+y[j+1])/2)<<print_out(bcL[j])<<print_out(bcR[j])<<endl; // write y grid
	outfile<<setw(6)<<ny+1<<print_out(y[ny])<<endl;
